...
(note: this is a minimally-modified version which gives a different normal form for negations.)
-/
...
-/
meta def tactic.interactive.contraposition (push : parse (tk "!" )?) :
  parse name_with_opt'? → tactic unit
| (some (h, h')) := get_local h >>= revert >> tactic.interactive.contraposition none >>
...
{ name       := "contraposition",
  category   := doc_category.tactic,
  decl_names := [`tactic.interactive.contraposition],
  tags       := ["logic"] }
